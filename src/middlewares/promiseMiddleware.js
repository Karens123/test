...
export default function promiseMiddleware (config = {}) {
	...
	return (_ref) => {
		...
		return next => (action) => {
			...
			/** Dispatch the first async handler. This tells the reducer that an async action has been dispatched */
			next({
				type: `${type}_${PENDING}`,
				...!!data ? { payload: data } : {},
				...!!meta ? { meta } : {}
			...
			const isAction = resolved => resolved &&
			(resolved.meta || resolved.payload);
			...
			action.payload.promise = promise.then(
				(resolved = {}) => {
					...
					return dispatch(isThunk(resolved)
				...
				(rejected = {}) => {
					...
					return dispatch(isThunk(rejected)
						? rejected.bind(null, resolveAction)
						: {
							...resolveAction,
