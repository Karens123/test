...
class NotificationRecord extends React.Component {
	static contextTypes = {
	...
	constructor (props) {
		...
		if (currentNotification) {
			this.state = {
				selectedRowKeys: [currentNotification.prodId],
			...
			if (rspInfo) {
				...
				if (`${QRY_PUSH_INFO_BY_FORM}_SUCCESS` === actionType) {
					if (rspInfo.resultCode !== Constant.REST_OK) {
						MsgUtil.showwarning(
							`查询通知记录列表失败,错误信息 ${rspInfo.resultCode} ${rspInfo.resultDesc}`);
					}
	...
	render () {
		...
		const columns = [
			{
				...
				render(text, pushInfoList) {
					return (
						<span>
							<a target="_blank" rel="noopener noreferrer" href={`${businessRoute.NotificationStatistic}${pushInfoList.responseId}`}  >
								<span>统计</span>
							</a>
						</span>
					);
		...
		return (
			<div>
				<Row>
					<Col span={22} >
						<span style={{ float: 'left', width: 900 }} >
...
const mapStateToProps = (state) => {
	...
	const { pushInfoList, currentPushInfo, } = state.get('PushService').
		toObject();
	return { rspInfo, pushInfoList, currentPushInfo, actionType, type: 1 };
};
...
export default connect(mapStateToProps, mapDispatchToProps)(NotificationRecord);
