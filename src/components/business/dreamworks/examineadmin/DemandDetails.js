...
class DemandDetails extends React.Component {
	//检验类型
	static contextTypes = {
	...
	static propTypes = {
		...
		currentDemandDetailInfo: PropTypes.object.isRequired,
	...
	constructor (props) {
		...
		if (!route) {
			...
			console.log('this.props.routeParams.worksId', this.props.routeParams.DemandId);
			this.state = {
				visible: false,
				WorksDetailsId: this.props.routeParams.DemandId,
				currentPage: 1,
				showModal: false,
				showData: {},
				modalType: undefined,
	...
	componentWillMount () {
		...
		actions.qryDemandDetailId(WorksDetailsId);
	}
	componentDidMount () {
	}
	...
	shouldComponentUpdate (nextProps, nextState) {
		const isRender = nextProps.WorksInfoList !== this.props.WorksInfoList
			...
			|| nextState.currentItme !== this.props.currentDemandDetailInfo
			|| nextState.showModal !== this.state.showModal;
		return isRender;
	}
	...
	failSubmit = (e) => {
		...
		if (auditResult && auditResult.length){
			this.setState({
				...
				const { currentDemandDetailInfo } = this.props;
	...
	okSubmit = (e) => {
		...
		this.setState({
			...
			const { operType,entityType,auditResult,ifSendSms } = this.state;
			const { currentDemandDetailInfo } = this.props;
			const entityId=currentDemandDetailInfo.worksInfo.worksId;
			console.log('___________form is submit,data is: ',{ operType,entityType,entityId,auditResult,ifSendSms });
			actions.auditDSNWork({ operType,entityType,entityId,auditResult,ifSendSms });
		});
		this.hideModal();
	...
	handleClearCurrentData = (e) => {
		...
		router.push(`${businessRoute.EnterExamineAdmin}${params.prepage?`/${params.prepage}`:''}`);
	};
	//弹窗内容控制0:不通过，1：通过
	renderModal = () => {
	...
	renderImgList = () => {
		const { currentDemandDetailInfo } = this.props;
		const item=[];
		for ( const its in currentDemandDetailInfo.worksPicList) {
			if (currentDemandDetailInfo.worksPicList[its].image) {
				item.push(
					<Col span={12}>
						<img
							src={currentDemandDetailInfo.worksPicList[its].image}
							onClick={() => { this.zoomPic(currentDemandDetailInfo.worksPicList[its].image); }}
							width={100}
							height={50}
							className="list-img"
						/>
	...
	renderForm = (deviceType,WorksFullInfo,currentDemandDetailInfo,defautImg,buyLink) => {
		...
		const DemandDetails={
			...
			//需求审核
		return (
			<div className="demandForm">
				<br /><br />
				<Col span={24}>
					<div style={{ display: 'flex',flexDirection: 'column',justifyContent: 'center',alignItems: 'center' }}>
						<img
							style={{ width: '100px',height: '100px',borderRadius: '50%' }}
							src={currentDemandDetailInfo && currentDemandDetailInfo.headImage ? currentDemandDetailInfo.headImage: defautImg}
							alt=""
							onClick={() => this.zoomPic(currentDemandDetailInfo && currentDemandDetailInfo.headImage ? currentDemandDetailInfo.headImage: defautImg)}
						/>
						<p>{123}</p>
					</div>
					<hr />
	...
	render () {
		const { WorksInfoList,currentDemandDetailInfo,params } = this.props;

		const  WorksFullInfo={ ...currentDemandDetailInfo.worksInfo };
		const  WorksPicBean={ ...currentDemandDetailInfo.worksPicList };
			...
			<div className="workDetails">
				...
				<Form>
					<Row gutter={20}>
						...
						{this.renderForm(deviceType,WorksFullInfo,currentDemandDetailInfo,defautImg,buyLink)}
					</Row>
				</Form>
			</div>
		);
...
const mapStateToProps = (state) => {
	...
	const { WorksInfoList,currentDemandDetailInfo  } = state.get('EnterExamineReducer').toObject();
	return {
		rspInfo,
		actionType,
		WorksInfoList,
...
export default connect(mapStateToProps, mapDispatchToProps)(Form.create()(DemandDetails));